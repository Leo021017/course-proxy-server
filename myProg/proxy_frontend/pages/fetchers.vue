...
export default {
    ...
    methods: {
        async update () {
            const data = await this.$http.get('fetcher/fetchers_status');
            this.fetchers = data.fetchers;
            this.lastupdate = moment().format('HH:mm:ss');
        },
        async clearStatus () {
            await this.$http.get('fetcher/clear_fetchers_status');
            this.$message.success('清空成功');
            await this.update();
        },
        async enableChange (fetcher) {
            if (fetcher.enable) {
                await this.$http.get('fetcher/fetcher_enable', { name: fetcher.name, enable: '0' });
            } else {
                await this.$http.get('fetcher/fetcher_enable', { name: fetcher.name, enable: '1' });
            }
