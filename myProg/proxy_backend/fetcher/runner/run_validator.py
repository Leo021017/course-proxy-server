...
def run_validators():
    ...
    in_que = Queue()    # 输入队列，用于接收验证任务
    out_que = Queue()   # 输出队列，用于返回验证结果
    ...
    while True:
        ...
        while not out_que.empty():
            ...
            db_op.pushValidateResult(proxy, success, latency)
        ...
        # 找一些新的待验证的代理放入队列中
        added_cnt = 0
        for proxy in db_op.getToValidate(VALIDATE_THREAD_NUM * 4):
            uri = f'{proxy.protocol}://{proxy.ip}:{proxy.port}'
