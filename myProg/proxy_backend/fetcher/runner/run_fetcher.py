'''
Author: Leo
Date: 2023-11-15 10:18:54
LastEditTime: 2023-11-16 17:07:17
FilePath: \proxy_backend\fetcher\runner\run_fetcher.py
...
def run_fetchers():
    ...
    while True:
        ...
        @func_set_timeout(30)   # 一个装饰器设置函数执行的超时时间
        def fetch_worker(fetcher):  # 传入一个爬取器去执行爬取任务，并返回爬取结果——>爬取到的代理结果列表
            f = fetcher()   
            proxies = f.fetch()
            return proxies
        ...
        threads = []
        que = Queue()   # 任务队列中是待存储的爬取结果
        for item in fetchers:   # 遍历所有爬取器
            data = db_op.getFetcher(item.name)
            if data is None:
        ...
        while not que.empty():
            ...
            for proxy in proxies:
                db_op.pushNewFetch(fetcher_name, proxy[0], proxy[1], proxy[2])
            db_op.pushFetcherResult(fetcher_name, len(proxies))
